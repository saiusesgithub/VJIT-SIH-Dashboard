...
export function NextReviewAction({ teamId, rounds }: { teamId: string; rounds: Array<{ id: string; number: number; name: string; status: ReviewStatus }> }) {
  ...
  const isAbsent = round.status === "absent";
  const actionLabel = round.status === "in_progress" ? "Continue" : isAbsent ? "Reopen" : "Start";
  return <section className="rounded-xl border border-zinc-200 bg-white p-4"><p className="text-xs font-semibold text-zinc-500">{round.status === "in_progress" ? "Current review" : isAbsent ? "Review marked absent" : "Next review"}</p><p className="mt-1 text-sm font-medium text-zinc-900">Review {round.number} · {round.name}</p><p className="mt-1 text-xs leading-5 text-zinc-500">{isAbsent ? "Open this review to continue if the team is now available." : "Check the team and venue above before proceeding. Opening this team page does not start a review."}</p><Link prefetch={false} href={`/judge/teams/${teamId}/reviews/${round.id}`} className="mt-3 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-lg bg-zinc-950 px-4 text-sm font-semibold text-white transition-colors hover:bg-zinc-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600">{actionLabel} Review {round.number}<ArrowRight className="size-4" /></Link></section>;
}
