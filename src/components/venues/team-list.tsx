...
import type { TeamListItem } from "@/types/domain";
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white">
      ...
      <div className="divide-y divide-zinc-100">
        {teams.map(({ team, problemStatement: problem, reviews, latestActivity, finalDecision }) => {
          return (
            <Link key={team.id} href={`/admin/teams/${team.id}`} className="group block px-5 py-4 transition-colors duration-150 hover:bg-zinc-50/70">
              <div className="grid gap-4 xl:grid-cols-[minmax(170px,1.3fr)_minmax(190px,1.5fr)_110px_110px_110px_105px_32px] xl:items-center xl:gap-3">
                <div><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-xs font-semibold text-zinc-500">{team.code}</span><span className="text-sm font-semibold text-zinc-950">{team.name}</span>{finalDecision ? <span className={`rounded-md border px-1.5 py-0.5 text-[10px] font-semibold ${finalDecision === "ELIMINATED" ? "border-red-200 bg-red-50 text-red-700" : finalDecision === "SHORTLISTED" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-700"}`}>{finalDecision === "SHORTLISTED" ? "Shortlisted" : finalDecision === "ELIMINATED" ? "Eliminated" : "Hold"}</span> : null}</div><p className="mt-1 text-xs text-zinc-400 xl:hidden">{problem.code} · {problem.title}</p></div>
                ...
                <div className="flex items-center gap-1.5 text-[11px] text-zinc-500"><Clock3 className="size-3 text-zinc-400" />{latestActivity ? formatTime(latestActivity) : "No activity"}</div>
