...
export default async function TeamPage({ params, searchParams }: { params: Promise<{ teamId: string }>; searchParams: Promise<{ requalification?: string }> }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {query.requalification ? <p role="status" className={`rounded-lg border px-4 py-3 text-sm ${query.requalification === "restored" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-red-200 bg-red-50 text-red-800"}`}>{query.requalification === "restored" ? "Team restored and assigned to the selected Day 2 room. Existing review data was retained." : query.requalification === "invalid" ? "This team is no longer eligible for restoration, or the selected Day 2 room is unavailable. Refresh and try again." : "The team could not be restored due to a temporary database error. Retry once; if it persists, check the Vercel function logs."}</p> : null}
