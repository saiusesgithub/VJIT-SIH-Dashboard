...
export default async function TeamPage({ params }: { params: Promise<{ teamId: string }> }) {
  const data = await getTeamPageData((await params).teamId);
  if (!data) notFound();
  const { team, venue, problemStatement: problem, judge, reviews } = data;
  ...
  return (
    <div className="space-y-6">
      ...
      <section><div className="mb-3"><h2 className="text-sm font-semibold text-zinc-950">Evaluation reviews</h2><p className="mt-0.5 text-xs text-zinc-500">Rubric scores, judge feedback, and submission history for all rounds.</p></div><div className="space-y-4">{reviews.map(({ review, round, rubric, judge: reviewJudge }) => <ReviewCard key={review.id} review={review} round={round} rubric={rubric} judge={reviewJudge} />)}</div></section>
    </div>
  );
}
