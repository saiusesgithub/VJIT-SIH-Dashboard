...
async function main() {
  ...
  const teams = await prisma.team.findMany({ where: { hackathonId: event.id }, orderBy: { teamCode: "asc" }, select: { teamCode: true, accessCodeHash: true, accessCodeLookup: true } });
  if (teams.length !== 48 || teams.some((team) => !team.accessCodeHash || !team.accessCodeLookup)) throw new Error("Expected 48 teams with complete access credentials.");
  const first = teams[0];
  const code = developmentTeamAccessCode(first.teamCode);
  if (!(await compare(code, first.accessCodeHash!)) || first.accessCodeHash === code || first.accessCodeLookup !== createTeamAccessLookup(code)) throw new Error("Team credential verification failed.");
  ...
  console.info(JSON.stringify({ teams: teams.length, completedReviews: totals, submissions, announcements, issues, rawCodesStored: false }));
}
