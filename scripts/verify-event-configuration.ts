...
const [venueRows, judgeRows, coordinatorRows, rubricRows, reviewRows, teamRows] = await Promise.all([
  ...
  sql.query(`SELECT COUNT(*)::int AS count FROM "Team" WHERE "hackathonId" = 'vjit-sih-2026'`),
]);
...
assert.deepEqual(reviewRows.map((row) => row.roundNumber), [1, 2, 3]);
assert.ok(reviewRows.every((row) => row.completed >= 0 && row.completed <= teamRows[0].count));
