...
Faculty can open **Admin → Team QR cards** (`/admin/qr-cards`), select all teams, a venue, or one team, and use **Print / Save PDF**. Cards contain team code/name, assigned venue/room, problem statement, and a QR code for `/judge/teams/<stable-internal-team-id>`. There are nine cards in a 3×3 grid per A4 sheet. Use 100% print scale and disable browser headers/footers. Test-scan a printed card with a phone before distributing a full batch. Reprint venue labels if a team moves rooms; the URL continues to resolve using its current assignment.
